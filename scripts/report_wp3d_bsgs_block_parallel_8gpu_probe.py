...
def run_one_worker(task_id, devices, seed):
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=len(tasks), mp_context=ctx) as ex:
        ...
        for fut in as_completed(futs):
            ...
            results.append(r)
            print(
                f"[done] task={r['task_id']} devices={tuple(r['devices'])} "
    ...
    out = {
        "experiment": "wp3d_bsgs_block_parallel_8gpu_probe",
    ...
    out_path = out_dir / "wp3d_bsgs_block_parallel_8gpu_probe_report.json"
