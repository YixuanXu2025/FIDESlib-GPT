...
def main():
    ...
    report = {
        "experiment": "wp4j_layered_ccmm_api",
        "purpose": "Validate formal GPU-only compute / materialize / decrypt API layering.",
        "n": n,
        "layers": layers,
        "slots": slots,
        "seed": seed,
    ...
    with HERuntime(cfg, rotation_steps=[]) as rt:
        ...
        print("[phase] GPU-only CCMM chain via rt.ccmm_gpu_fused_raw", flush=True)
        compute_times = []
        compute_states = []

        for ell, U in enumerate(Us):
            print(f"[ccmm_gpu_fused_raw] layer={ell}", flush=True)
            t_layer0 = now_us()
            rows = rt.ccmm_gpu_fused_raw(rows, U.astype(np.float64).tolist())
            t_layer1 = now_us()
    ...
    report.update({
        ...
        "errors": {
            "gpu_only_layered_api_to_ref_max_abs_err": err,
        },
    ...
    print("=" * 100)
    print("WP4-J layered CCMM API report")
    print(json.dumps(report, ensure_ascii=False, indent=2))

    out_dir = Path("/workspace/FIDESlib-GPT/reports")
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / "wp4j_layered_ccmm_api_report.json"
