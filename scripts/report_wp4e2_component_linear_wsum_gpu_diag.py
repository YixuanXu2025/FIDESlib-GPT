...
def main():
    ...
    report = {
        "experiment": "wp4e2_component_linear_wsum_gpu_diag",
        "n_in": n_in,
        "slots": slots,
        "seed": seed,
    ...
    with HERuntime(cfg, rotation_steps=[]) as rt:
        ...
        dec_in_errors = []
        for ct in ct_rows:
            y, err = safe_decrypt(rt, ct, slots)
            dec_in.append(None if y is None else y.tolist())
            dec_in_errors.append(err)
        ...
        dec_gpu, gpu_err = safe_decrypt(rt, ct_gpu, slots)
        dec_high, high_err = safe_decrypt(rt, ct_high, slots)
        t6 = now_us()
    ...
    report.update({
        ...
        "decrypt_input_errors": dec_in_errors,
        "decrypt_gpu_error": gpu_err,
        "decrypt_highlevel_error": high_err,
        "dec_gpu": None if dec_gpu is None else dec_gpu.tolist(),
        "dec_high": None if dec_high is None else dec_high.tolist(),
        "y_ref": y_ref.tolist(),
    ...
    if dec_gpu is not None:
        report["err_gpu_to_ref"] = float(np.max(np.abs(dec_gpu - y_ref)))
    if dec_high is not None:
        report["err_high_to_ref"] = float(np.max(np.abs(dec_high - y_ref)))
    if dec_gpu is not None and dec_high is not None:
        report["err_gpu_to_high"] = float(np.max(np.abs(dec_gpu - dec_high)))

    out_dir = Path("/workspace/FIDESlib-GPT/reports")
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / "wp4e2_component_linear_wsum_gpu_diag_report.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(report, f, ensure_ascii=False, indent=2)

    print("=" * 100)
    print("WP4-E2 GPU-native component linear weighted-sum DIAG")
    print("=" * 100)
    ...
    print(high_state)
    print("-" * 100)
    print(f"decrypt_gpu_error={gpu_err}")
    print(f"decrypt_highlevel_error={high_err}")
    if "err_gpu_to_ref" in report:
        print(f"err_gpu_to_ref={report['err_gpu_to_ref']:.6e}")
    if "err_high_to_ref" in report:
        print(f"err_high_to_ref={report['err_high_to_ref']:.6e}")
    if "err_gpu_to_high" in report:
        print(f"err_gpu_to_high={report['err_gpu_to_high']:.6e}")
    print("=" * 100)
    print("报告已保存")
