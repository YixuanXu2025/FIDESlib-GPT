...
def make_controlled_rowwise_bundle(rt, matrix, a_scalar=1, ring_dim=1 << 14, batch_size=8):
    ...
    return bundle, info, template_ct, sk_towers, moduli
...
def phase_aware_logical_n_cmt_oracle(rt, bundle, n, a_scalar, label):
    ...
    Steps:
      1. decrypt current bundle rows to plaintext matrix M
      2. transpose M logically
      3. re-encode each output row as controlled RLWE:
           c1 = a
           c0 = M.T[row] - a*sk
    """
    ...
    trace = {
        ...
        "a_scalar": int(a_scalar),
        "warning": "debug-only decrypt-transpose-reencrypt oracle; not secure/homomorphic",
    }

    return out_bundle, trace
...
def run_case(rt, name, U, a_scalar):
    ...
    bundle, build_info, _, _, _ = make_controlled_rowwise_bundle(
        rt,
        U.tolist(),
        a_scalar=a_scalar,
    )
    ...
    cmt_bundle, cmt_trace = phase_aware_logical_n_cmt_oracle(
        rt=rt,
        bundle=bundle,
        n=n,
        a_scalar=a_scalar,
        label=f"phase_aware_CMT_a{a_scalar}",
    )
    ...
    return {
        ...
        "expected_phase_aware_cmt_output_U_transpose": U_T.tolist(),
        "build_info": build_info,
        "input_decrypt": input_dec,
        "phase_aware_cmt_trace": cmt_trace,
        "phase_aware_cmt_decrypt": cmt_dec,
        "analysis": {
            "input_decrypt_exact": compare_matrix(input_dec["decoded"], U),
            "phase_aware_cmt_equals_U_transpose": compare_matrix(cmt_dec["decoded"], U_T),
        },
    }
...
def main():
    n = 4
    rng = np.random.default_rng(209200)
    ...
    report = {
        "experiment": "wp4y2h_phase_aware_cmt_oracle_probe",
        "purpose": (
            "Test a phase-aware logical-n C-MT oracle for controlled RLWE c1!=0 inputs. "
            "Unlike raw component transpose, this oracle decrypts the phase, transposes plaintext, "
            "and re-encodes controlled RLWE rows."
        ),
        "status": "debug_phase_aware_cmt_oracle_not_secure",
        "n": n,
        "warning": {
            "value": "This oracle decrypts and re-encrypts with secret key. It is not secure and not homomorphic.",
            "zh": "该 oracle 使用 secret key 解密再重构，只用于定位语义，不是最终 C-MT。",
        },
        "cases": [],
    }
    ...
    report["checks"] = {
        "all_controlled_inputs_decrypt_exact（所有 controlled RLWE 输入都能解密回 U）": all(
            c["analysis"]["input_decrypt_exact"]["exact"]
            for c in report["cases"]
        ),
        "all_phase_aware_cmt_outputs_equal_transpose（phase-aware C-MT oracle 全部等于 U^T）": all(
            c["analysis"]["phase_aware_cmt_equals_U_transpose"]["exact"]
            for c in report["cases"]
        ),
        "any_phase_aware_cmt_case_passes（至少一个 phase-aware C-MT case 正确）": any(
            c["analysis"]["phase_aware_cmt_equals_U_transpose"]["exact"]
            for c in report["cases"]
        ),
        "failed_cases（失败 case 摘要）": [
            {
                "name": c["name"],
                "a_scalar": c["a_scalar"],
                "max_abs_err": c["analysis"]["phase_aware_cmt_equals_U_transpose"]["max_abs_err"],
                "l1_err": c["analysis"]["phase_aware_cmt_equals_U_transpose"]["l1_err"],
            }
            for c in report["cases"]
            if not c["analysis"]["phase_aware_cmt_equals_U_transpose"]["exact"]
        ],
    }

    if report["checks"]["all_phase_aware_cmt_outputs_equal_transpose（phase-aware C-MT oracle 全部等于 U^T）"]:
        report["global_decision"] = {
            "value": "phase_aware_logical_n_cmt_oracle_works",
            "next_step": "WP4-Y2i",
            "next_goal": "Use controlled RLWE inputs + phase-aware C-MT oracle in full CCMM oracle pipeline.",
            "zh": "phase-aware C-MT oracle 对非透明 RLWE 输入成立；下一步接入完整 CCMM oracle pipeline。",
        }
    else:
        report["global_decision"] = {
            "value": "phase_aware_cmt_oracle_failed",
            "next_step": "WP4-Y2i",
            "next_goal": "Diagnose controlled RLWE reconstruction after transpose.",
            "zh": "即使 phase-aware oracle 也失败，需要检查转置后重构。",
        }

    out_dir = Path("/workspace/FIDESlib-GPT/reports")
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / "wp4y2h_phase_aware_cmt_oracle_probe_report.json"
    out_path.write_text(json.dumps(report, ensure_ascii=False, indent=2, default=str), encoding="utf-8")

    print("=" * 100)
    print("WP4-Y2h phase-aware logical-n C-MT oracle probe")
